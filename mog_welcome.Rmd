...
variable_list <- read_csv("data/lsoa_dataset_labelsv3.csv")
# 214 variables as of NI update

```


<h1>Welcome to __MOG__ (alpha), the Mapping Overlaps Gadget for local authorities in England and Wales, and Health and Social Care Trusts in Northern Ireland.</h1>

<br>

__MOG__ is a Shiny app for visualising neighbourhood level data in England and some of Wales by Local Authority. It combines a collection of open data from the Office of National Statistics (ONS), the Consumer Data Research Centre (CDRC), the Department for Communities and Local Government (DCLG), and the Department for Transport (DfT), StatsWales, and NISRA (Northern Ireland).

All data is at the [Lower Super Output Area](https://www.ons.gov.uk/methodology/geography/ukgeographies/censusgeography){target="_blank"} level (or equivalent in NI or Wales). There are 104 variables for England, 82 variables for Wales, and 45 shared variables between England and Wales. There are 50 variables for Northern Ireland. At the moment, the data included comes from the following government open data sources:
